...
/**
 * 数据类型 1） 按照数据类型分类 基本数据类型【primitive type】 数值型 整数（byte，short，int，log）→ 通常使用 int
 * 浮点（float，double） 字符型（char） 布尔型（boolena）
 * 
 * 引用数据类型【reference type】 类（class）→ 字符串在这里 接口（interface） 数组（[]）
 */
// package basic;
